...
Cli.prototype.createTplDir = async function (pathName) {
  try {
    const exist = await fse.pathExists(pathName);
    if (exist) {
      return true;
    }
  } catch (e) {
...
Cli.prototype.viewTplList = function () {
  ...
  const choices = arrName.map((item) => {
    ...
    return {
      name: `${item}${_description ? `: ${_description}` : ''}`,
      value: item,
    };
  });
...
Cli.prototype.compile = async function (dataTpl = [], data = {}) {
  ...
  for (let i = 0; i < dataTpl.length; i++) {
    const { from, to, disabledEjs = false, dot = false } = dataTpl[i];
    ...
    if (exists) {
      ...
      // 不使用 ejs
      if (disabledEjs) {
        await memFsE.copy(compileFrom, compileTo, options);
      } else {
        await memFsE.copyTpl(compileFrom, compileTo, renderData, {}, options);
      }
    }
  }
