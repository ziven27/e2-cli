...
function Cli() {
  ...
  // 用户输入命令时创建的项目目录
  this.pathConfig = path.join(cwd, G_DATA.config.fileName);

  // 隐藏的数据
  this.pathData = path.join(cwd, G_DATA.data.fileName);
 ...
 */
Cli.prototype.createTplDir = async function () {
  const to = path.join(this.cwd, 'e2-tpl');
  ...
  console.log(`已为你创建默认存放模版的文件夹：${to}`);
  return true;
};
 ...
 */
Cli.prototype.getFileData = async function () {
  const fildData = await fse.readJson(this.pathData);
  // 数据已经存在直接返回
  if (fildData) {
    return fildData;
  }
  ...
  // 文件没有读取成功，直接用默认文件创建
  await fse.writeJson(G_DATA.data.fileName, data_default, {
    spaces: 2,
  });
  console.log(`已为你添加数据文件: ${G_DATA.data.fileName}`);
  return data_default;
};
 ...
 * @returns {Promise<unknown>}
 */
Cli.prototype.getFileConfig = function () {
  return new Promise((resolve) => {
    // 文件没有读取成功，直接用默认文件创建
    const writeFile = () => {
      fse
        .writeJson(G_DATA.config.fileName, G_DATA.config.data, {
          spaces: 2,
        })
        .then(() => {
          console.log(`已为你添加默认配置文件: ${G_DATA.config.fileName}`);
          resolve(G_DATA.config.data);
        })
        .catch((err) => {
          console.log(err);
        });
    };
    fse.readJson(this.pathConfig).then(resolve).catch(writeFile);
  });
};
 ...
 * @returns {Promise<unknown>}
 */
Cli.prototype.getRenderDataByTplName = function (tplName, appendData = {}) {
  return new Promise((resolve, reject) => {
    this.getFileData()
      .then((fileData) => {
        const {
          _data = {},
          [tplName]: { _data: tplData = {} } = {},
        } = this.fileConfig;
        const renderData = {
          // 私有默认数据
          ...G_DATA.private,
...
Cli.prototype.useTpl = async function (tplName) {
  ...
  const renderData = await this.getRenderDataByTplName(tplName, appendData);
